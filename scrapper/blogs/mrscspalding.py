...
def main():
    source = requests.get('http://mrscspalding.blogspot.com/').text
    ...
    for article in articles:
        ...
        for comment in comments:
            ...
            user = comment.find('div', class_='comment-header').cite.text.replace('\n', '').replace('\t', '')
            if user == 'Unknown' or user == 'Anonymous':
                user = 'Anonymous_user_' + str(anonymous)
                anonymous = anonymous + 1

            date = comment.find('span', class_='datetime secondary-text').text.replace('\n', '').replace('\t', '')
            ...
            while fail_condition:
                try:
                    ...
                    for reply in replies:
                        ...
                        user = reply.find('div', class_='comment-header').cite.text.replace('\n', '').replace('\t', '')
                        if user == 'Unknown' or user == 'Anonymous':
                            user = 'Anonymous_user_' + str(anonymous)
                            anonymous = anonymous + 1

                        date = reply.find('span', class_='datetime secondary-text').text.replace('\n', '').replace('\t', '')
                        parsed_date = parse(date)
                        timestamp = datetime.datetime.timestamp(parsed_date)
